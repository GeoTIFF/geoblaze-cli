...
const urlToGeoTIFF = "http://127.0.0.1:8080/spam2005v2r0_production_wheat_rainfed.tiff";

const relativePathToGeoJSON = "test/data/ukraine.geojson"
const urlToGeometry = "http://127.0.0.1:8080/ukraine.geojson";
...
describe('Max', function() {
  ...
  it("Should calculate maximum of a GeoTIFF", async () => {
    ...
    expect(stdout).contains("Highest Pixel Value (Max) is 42,256.2");
  });
  ...
  it("Should calculate maximum of a GeoTIFF within a local GeoJSON", async () => {
    ...
    expect(stdout).contains("Highest Pixel Value (Max) is 8,979.8");
  });
});
*/
describe('Mean Average', function() {
  ...
  it("Should calculate mean average of a GeoTIFF", async () => {
    ...
    expect(stdout).contains("Mean of Pixel Values is 1,700.03");
  });
...
describe('Median Average', function() {
  ...
  it("Should calculate median average of a GeoTIFF", async () => {
    ...
    expect(stdout).contains("Median Pixel Value (Max) is 0");
  });
...
describe('Mode Average', function() {
  this.timeout(timeout);
  it("Should calculate mode average of a GeoTIFF", async () => {
